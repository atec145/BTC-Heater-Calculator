...
domain/        Pure Kotlin — no Android imports
  model/       MinerConfig, MarketData, HourlyPrice, SaleMode, ProfitabilityResult, DifficultyEpoch
...
data/
  remote/      Retrofit APIs: AwattarApi, BlockchainInfoApi (in MempoolApi.kt), CoinGeckoApi, HeizOel24Api, MempoolSpaceApi
...
presentation/
  ...
  screens/difficulty/  Difficulty chart — DifficultyViewModel, DifficultyScreen, DifficultyUiState
  screens/settings/   Miner config + sale mode + oil settings — SettingsViewModel, SettingsScreen
  components/         PriceBarChart, DifficultyLineChart (both pure Canvas, no external chart lib)
  navigation/         AppNavigation with Screen sealed class (Home, Difficulty, Settings); bottom nav bar
...
- mempool.space: `https://mempool.space/` — `MempoolSpaceApi.getDifficultyAdjustments()` returns `JsonArray` of `[timestamp, blockHeight, difficulty, changePercent]`
...
- `PriceBarChart` and `DifficultyLineChart` use pure Canvas (no external chart library) — keep it that way
